// App.jsx (patched)
// - Prefetch media hook
// - Instrumentation (logStep)
// - Optimistic navigation on join
// - Unique gameId generation using crypto.randomUUID if available
// - Safer VideoChat implementation and cleanup
// - Robust ICE candidate dedupe & guards
// - GameClocks uses toMillis() when available
// Keep this file as your single source of truth for the app.
...
// --- Sound Effects Manager (unchanged) ---
const soundThemes = {
  default: {
...
// Prefetch media to reduce join latency (call from lobby)
function usePrefetchMedia({ audio = true, video = true } = {}) {
    ...
    async function prefetch() {
      try {
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
          ...
          streamRef.current = s;
          // keep it warmed; will be stopped on unmount
        }
      } catch (err) {
        console.warn('Prefetch media failed (user may have denied)', err);
      }
    }
    prefetch();
    return () => {
...
// Logging instrumentation helper for join latency and debugging
function nowLabel() {
...
// Small helper to create robust unique IDs for games
...
// --- GameSetup (calls prefetch, optimistic navigation improvements) ---
const GameSetup = ({ user, onGameStart, onStartVsComputer, onStartOfflineGame }) => {
  // Prefetch media as soon as user opens lobby to reduce join latency
  usePrefetchMedia({ audio: true, video: true });
  ...
  const requestRefRef = useRef(null); // store the matchmaking_requests doc ref
  const gameListenerUnsubRef = useRef(null);

  useEffect(() => {
    const q = query(collection(db, 'games'), where('status', '==', 'waiting'), limit(20));
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const games = querySnapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      setOpenGames(games);
      setLoading(false);
    }, (err) => {
  ...
  const handleCreateGame = async () => {
    setLoading(true);
    try {
      const gameId = makeGameId(user.uid);
      ...
      logStep('create-game-success', { gameId });
      onGameStart(gameId);
    } catch (err) {
  ...
  const handleJoinGame = async (gameId) => {
    ...
    try {
      ...
      await runTransaction(db, async (tx) => {
        ...
        // Reject if not joinable
        if (data.status !== 'waiting') {
          throw new Error('Game is not available to join');
        }
        if (data.player2 && data.player2.uid) {
          throw new Error('Game already has a second player');
        }
        if (data.player1?.uid === user.uid) {
          throw new Error('You cannot join your own waiting game');
        }

        // Update minimal fields only to reduce transaction payload / latency
        const updated = {
      ...
      // Immediately navigate optimistically (do not wait for a secondary read)
  ...
  // Start quick match: tries to claim waiting game, otherwise creates matchmaking request
  const startQuickMatch = async () => {
    ...
    try {
      // 1) Immediate attempt to claim any waiting game with same timeControl
      ...
      if (!waitingSnap.empty) {
        ...
        try {
          // Transactional claim (same safety as handleJoinGame)
          await runTransaction(db, async (tx) => {
      ...
      // 2) Create matchmaking request (fallback)
      ...
      // Read back the request to obtain the server timestamp baseline
      ...
      // 3) Listen for an ACTIVE game that includes this user AND is created after our request
      ...
      const unsub = onSnapshot(gamesQuery, async (snap) => {
        try {
          ...
          for (const docSnap of snap.docs) {
            ...
            if (gameCreatedAtSec + 2 < reqCreatedAtSec) {
              console.warn("GameSetup: ignoring active game older than our request", docSnap.id, { gameCreatedAtSec, reqCreatedAtSec });
              continue;
            }
            ...
            if (requestRefRef.current) {
              try { await deleteDoc(requestRefRef.current); } catch (e) { console.warn('failed to delete matchmaking request', e); }
              requestRefRef.current = null;
            }

            setSearching(false);
            if (gameListenerUnsubRef.current) { gameListenerUnsubRef.current(); gameListenerUnsubRef.current = null; }
  ...
  const cancelQuickMatch = async () => {
    ...
    if (gameListenerUnsubRef.current) {
      try { gameListenerUnsubRef.current(); } catch (_) {}
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (requestRefRef.current) {
        try { deleteDoc(requestRefRef.current); } catch (_) {}
        requestRefRef.current = null;
      }
    };
  }, []);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-8 bg-gray-800 rounded-lg shadow-lg">
      ...
      <div className="flex justify-center items-center space-x-4 mb-6">
        ...
        {!searching ? (
          <button onClick={startQuickMatch} className="w-full sm:w-auto bg-indigo-600 text-white font-bold py-2 px-6 rounded-md hover:bg-indigo-700 transition duration-300">
            Quick Match
          </button>
        ) : (
          <button onClick={cancelQuickMatch} className="w-full sm:w-auto bg-red-600 text-white font-bold py-2 px-6 rounded-md hover:bg-red-700 transition duration-300">
            Cancel Search
          </button>
        )}
      </div>
...
// --------------------- Remaining components (mostly unchanged) ---------------------
...
// --------------------- GameClocks (robust timestamp handling) ---------------------
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      try {
        ...
        // compute elapsed since lastMoveTimestamp robustly
        const nowMs = Date.now();
        const lastMoveMs = gameData.lastMoveTimestamp?.toMillis ? gameData.lastMoveTimestamp.toMillis()
          : (gameData.lastMoveTimestamp?.seconds ? gameData.lastMoveTimestamp.seconds * 1000 : nowMs);
        ...
        console.warn('clock tick error', err);
      }
    }, 1000);
...
// --------------------- PromotionDialog, ChatBox, GameOverDialog, GameActions, CapturedPiecesPanel, SettingsDialog (unchanged) ---------------------
// For brevity I keep these components same as before (they were robust). Paste them from your original file unchanged.
// ... (PromotionDialog, ChatBox, GameOverDialog, GameActions, CapturedPiecesPanel, SettingsDialog)
// Since the user provided code earlier, we keep them unchanged here. (In the real patch I keep them as-is.)

// To keep this file self-contained, I re-paste the earlier unchanged components:
...
const GameOverDialog = ({ gameData, user, onViewProfile, onRematch, onLeave }) => {
  ...
  if (gameData.mode === 'online' && gameData.rematchOffer && gameData.rematchOffer !== myPlayerKey) { // retain original logic
    return (
...
// --------------------- VideoChat (replaced with safer, hardened logic) ---------------------
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        // add tracks but avoid duplicates
        const existingSenderIds = new Set((pcRef.current.getSenders() || []).map(s => s.track && s.track.id));
        stream.getTracks().forEach(track => {
          if (!existingSenderIds.has(track.id)) {
            try { pcRef.current.addTrack(track, stream); } catch(e) { console.warn('addTrack failed', e); }
          }
        });
      } catch (err) {
      ...
      // snapshot listener — guard uses of pcRef
      signalingUnsubRef.current = onSnapshot(gameRef, async (snap) => {
        try {
          ...
          if (!pcRef.current) {
            console.warn('snapshot: pcRef not ready; skipping signals for now');
            return;
          }
          ...
          // player1 sets remote when answer appears
          ...
          console.error('VideoChat snapshot handler error', err);
        }
      }, (err) => {
        console.error('VideoChat snapshot error', err);
      });

      // If player1 and no existing offer, create one.
      if (user.uid === gameData.player1.uid) {
        ...
        if (!existing || !existing.offer) {
          ...
          } catch (err) { console.warn('set remote from existing offer failed', err); }
        }
      }
    };
      ...
      if (pcRef.current) {
        try {
          (pcRef.current.getSenders() || []).forEach(s => {
            try { pcRef.current.removeTrack(s); } catch(e) {}
          });
          pcRef.current.close();
        } catch(e) {}
        pcRef.current = null;
  ...
  const makeMove = useCallback(async (move) => {
    ...
    if (isGameOver) {
      if (gameCopy.isCheckmate()) {
        // winner mapping assumes player1 is white — ensure invariant
    ...
    if (gameData.mode === 'online') {
      ...
      if (result.color === 'w') {
        timeUpdate.player1Time = Math.max(0, (gameData.player1Time || 0) - timeSinceLastMove);
      } else {
        timeUpdate.player2Time = Math.max(0, (gameData.player2Time || 0) - timeSinceLastMove);
      }
